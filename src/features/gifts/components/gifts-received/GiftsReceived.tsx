import React, { useState, useEffect } from 'react';
import moment from 'moment';
import { MaterialUiPickersDate } from '@material-ui/pickers/typings/date';
import {
  Box,
  Card,
  Container,
  Divider,
  Grid,
  Typography,
} from '@material-ui/core';
import { Alert, Skeleton } from '@material-ui/lab';
import GiftsFilters from '../gifts-filters/GiftsFilters';
import GiftsReceivedList from '../gifts-received-list/GiftsReceivedList';
import {
  GiftsReceivedReduxActions,
  GiftsReceivedReduxProps,
} from './interfaces';
import {
  EMPTY_MESSAGES,
  ERROR_MESSAGES,
} from '../../../../core/constants/general-messages';
import './GiftsReceived.css';

type GiftsReceivedProps = GiftsReceivedReduxActions & GiftsReceivedReduxProps;

const GiftReceived = ({
  giftCancel,
  giftsReceived,
  status,
  cancelGift,
  getAllGiftsReceived,
  getStatus,
  clearGiftCancel,
}: GiftsReceivedProps): JSX.Element => {
  const initialFromDate = moment().subtract(7, 'days').format('YYYY-MM-DD');
  const initialToDate = moment().format('YYYY-MM-DD');

  const [filters, setFilters] = useState({
    fromDate: initialFromDate,
    toDate: initialToDate,
    statusId: '',
  });

  useEffect(() => {
    const { fromDate, toDate, statusId } = giftsReceived.filters;

    if (!fromDate && !toDate && !statusId) {
      getStatus();
      getAllGiftsReceived({
        fromDate: initialFromDate,
        toDate: initialToDate,
        statusId: '',
      });
    } else {
      setFilters({
        fromDate,
        toDate,
        statusId,
      });
    }
  }, [
    getAllGiftsReceived,
    getStatus,
    initialFromDate,
    initialToDate,
    giftsReceived.filters,
  ]);

  useEffect(() => {
    const { loading, isSaveCompleted, error } = giftCancel;

    if (!loading && !error && isSaveCompleted) {
      getAllGiftsReceived(filters);
      clearGiftCancel();
    }
  }, [getAllGiftsReceived, giftCancel, filters, clearGiftCancel]);

  const handleFromDate = (date: MaterialUiPickersDate) => {
    setFilters({ ...filters, fromDate: date?.format('YYYY-MM-DD') || '' });
  };

  const handleToDate = (date: MaterialUiPickersDate) => {
    setFilters({ ...filters, toDate: date?.format('YYYY-MM-DD') || '' });
  };

  const handleStatusChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setFilters({
      ...filters,
      statusId: (event.target as HTMLInputElement).value,
    });
  };

  const handleCancelGift = (id: string) => {
    cancelGift(id);
  };

  const { loading, data, isFetchCompleted, error } = giftsReceived;

  const getGifts = () => {
    if (loading)
      return (
        <>
          {[0, 1, 2, 3, 4, 5].map((val) => {
            return (
              <Card key={val} style={{ marginBottom: '15px' }}>
                <Box
                  display="flex"
                  flexDirection="column"
                  p={[2, 3]}
                  justifyContent="center">
                  <Box display="flex">
                    <Skeleton width="10%" style={{ marginRight: 10 }}>
                      <Typography variant="h6">.</Typography>
                    </Skeleton>
                    <Skeleton width="60%">
                      <Typography variant="h6">.</Typography>
                    </Skeleton>
                  </Box>
                  <Skeleton width="50%" />
                  <Skeleton width="50%" />
                  <Divider
                    style={{ backgroundColor: '#eee', margin: '10px 0' }}
                  />
                  <Box display="flex" py={1}>
                    <Skeleton width="20%" style={{ marginRight: 10 }} />
                    <Skeleton width="50%" />
                  </Box>
                </Box>
              </Card>
            );
          })}
        </>
      );

    if (!loading && error)
      return <Alert severity="error">{ERROR_MESSAGES.FETCH_ERROR}</Alert>;

    if (isFetchCompleted && !data.length)
      return (
        <Alert severity="info" variant="outlined">
          {EMPTY_MESSAGES.GIFTS}
        </Alert>
      );

    return (
      <GiftsReceivedList
        giftsReceived={data}
        handleCancelGift={handleCancelGift}
        giftCancel={giftCancel}
      />
    );
  };

  return (
    <Container>
      <Grid container justify="center" alignItems="center">
        <Grid item xs={12} md={8}>
          <Box
            display="flex"
            justifyContent="space-between"
            alignItems="center"
            py={3}>
            <Typography variant="h5">Regalos recibidos</Typography>
            <GiftsFilters
              filters={filters}
              handleFromDate={handleFromDate}
              handleToDate={handleToDate}
              handleStatusChange={handleStatusChange}
              status={status}
              getGifts={getAllGiftsReceived}
            />
          </Box>
          {getGifts()}
        </Grid>
      </Grid>
    </Container>
  );
};

export default GiftReceived;
